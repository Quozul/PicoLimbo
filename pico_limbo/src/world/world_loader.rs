use blocks_report::{BlocksReport, BlocksReportId, build_report_mappings};
...
) -> Result<(blocks_report::InternalMapping, Option<Vec<BlocksReportId>>)> {
...
fn load_schematic(
    ...
    custom_mapping: Option<Vec<BlocksReportId>>,
) -> Result<(Schematic, Option<Vec<BlocksReportId>>)> {
    let schem = Schematic::load_schematic_file(schematic_path, internal)?;
    Ok((schem, custom_mapping))
}
...
pub struct LoadWorldResult {
    ...
    pub custom_mapping: Option<Vec<BlocksReportId>>,
}
...
pub fn try_load_world(
    ...
    let (schematic, mapping) = time_operation("Loading schematic", || {
        load_schematic(&schematic_path, &internal_mapping, custom_mapping)
    })?;
