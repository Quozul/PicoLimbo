...
pub async fn send_play_packets(client: Client, state: ServerState) -> Result<(), HandlerError> {
    ...
    let void_biome_index = get_the_void_index(protocol_version, state.data_directory()) as i32;

    let structure = Structure::load_structure_file(Path::new(state.structure()), protocol_version)
        .map_err(|err| {
            error!("{err}");
        })
        .unwrap();
