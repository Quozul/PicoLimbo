...
use reports::registries_report::RegistriesReport;
use std::path::Path;
...
fn main() {
    ...
    let resource_root = Path::new("data/generated/V1_21_11-pre2");
    let registry_manager = RegistryManager::from_resource_path(resource_root);

    let registry = registry_manager
        .get_optional(RegistryKeys::DimensionType)
...
fn get_dimension_info(
    ...
    registry
        .and_then(|reg| reg.get_optional(dimension_name))
        .and_then(|element| {
