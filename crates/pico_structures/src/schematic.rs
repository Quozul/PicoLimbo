...
use crate::palette_translator::PaletteTranslator;
...
/// Holds the result of parsing the schematic's palette.
struct PaletteData {
    /// The final, consolidated palette of global block state IDs.
    palette: Vec<VarInt>,
    /// A map from the schematic's internal palette IDs to the local indices of our final palette.
    schematic_id_to_local_index: HashMap<i32, i32>,
    /// The local index in our palette that represents air.
    air_index: i32,
}

/// A parsed representation of the schematic's block data.
struct BlockData {
...
pub struct Schematic {
    ...
    offset: (i32, i32, i32),
    solid_blocks_count: usize,
    /// The local palette index for an air block.
    air_index: i32,
}
...
impl Schematic {
    ...
    pub fn load_schematic_file(
        ...
        let palette_data = Self::build_palette_mapping(&nbt, version)?;
        let block_data = Self::build_block_lookup(&nbt, &palette_data, dimensions)?;
        ...
        Ok(Self {
            ...
            air_index: palette_data.air_index,
        })
    }
    ...
    fn build_palette_mapping(
        ...
        let air_index = 0;
        let mut translator = PaletteTranslator::new(Self::AIR_BLOCK_STATE_ID);
        let mut schematic_id_to_local_index = HashMap::new();
        ...
        for block_tag in palette_nbt {
            if let Some(schematic_palette_id) = block_tag.get_int() {
                ...
                let local_index = translator.get_or_insert(global_id);
                schematic_id_to_local_index.insert(schematic_palette_id, local_index);
            }
        }

        let palette = translator.palette_ids.iter().map(VarInt::from).collect();

        Ok(PaletteData {
            palette,
            schematic_id_to_local_index,
            air_index,
        })
    }

    /// Parses the `BlockData` array and constructs a lookup map of block positions to palette indices.
    fn build_block_lookup(
        nbt: &Nbt,
        palette_data: &PaletteData,
        dimensions: (i32, i32, i32),
    ) -> Result<BlockData, SchematicError> {
        ...
        while reader.remaining() > 0 {
            ...
            let local_index = palette_data
                .schematic_id_to_local_index
                .get(&schematic_block_id)
                .copied()
                .unwrap_or(palette_data.air_index);

            if local_index != palette_data.air_index {
                let y = index / (width * length);
                let z = (index % (width * length)) / width;
                let x = index % width;
                lookup.insert((x, y, z), local_index);
    ...
    /// Returns the palette index for an air block.
    pub fn get_air_index(&self) -> i32 {
        self.air_index
    }
}
